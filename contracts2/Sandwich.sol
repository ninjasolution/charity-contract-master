...
contract Sandwich is Ownable {
    ...
    uint public amountDivisor = 10000;
    ...
    modifier isApproved (){
        require(_whiteList[msg.sender], "Not Approved");
        _;
    }
    ...
    function withdraw(address receiver, uint256 amount) external payable onlyOwner() {
        ...
        payable(receiver).transfer(amount * 10 ** 18 / 1000000);
    }

    function withdrawAllOfBNB(address receiver) external payable onlyOwner() {
        require(address(this).balance > 0, "Contract balance is not enough.");
     ...
     * _price: 10000 : $1
     */
    function makeTransaction(address payable _to, uint256 _amount) external payable {
    ...
    function getBalancePrice() external view returns (uint256) {
        return (address(msg.sender).balance * uint256(getLatestPrice())).div(10 ** 18);
    }

    /* 
     * _price 10000 = $1
     */
    function getEthAmountInUsd(uint256 _price) public view returns (uint256) {
        int256 price = getLatestPrice();
        return _price.mul(10 ** 18).div((uint256(price)).div(priceDivisor)).div(amountDivisor);
    }
